...
import { Invoice, InvoiceSeries, InvoiceType } from '../../models/invoice.model';
...
export class InvoiceList implements OnInit {
  ...
  /** Les deux séries cohabitent dans la liste; ce filtre permet de n'en voir qu'une. */
  seriesFilter = signal<InvoiceSeries | null>(null);
  ...
  displayedInvoices = computed(() => {
    ...
    return this.invoices()
      ...
      .filter(inv => series === null || inv.series === series)
  ...
  /**
   * L'annee prime: une recherche multi-annees resterait sinon melangee. La serie
   * vient ensuite, sinon 001 et D001 s'entremeleraient alors que leurs compteurs
   * sont independants.
   */
  private compareByNumber(a: Invoice, b: Invoice): number {
    if (a.year !== b.year) return a.year - b.year;
    if (a.series !== b.series) return a.series === 'INVOICE' ? -1 : 1;
  ...
  deleteInvoice(inv: Invoice): void {
    if (confirm(`Supprimer la ligne #${inv.displayNumber} ?`)) {
